...
//-- Judgement for Auto mode Car direction with UltraSound sensor  自动驾驶下车的信息
volatile int MotorMode = TurnOff ; //define current Motor operation mode  默认马达是关闭状态,小车停止不走
volatile int LeftMotorSpeed = 130 ;// Initially set up 50% speed  ===  168
volatile int RightMotorSpeed = 130 ;
volatile int MotorPowerLevel = 10; // send to mobile device to show(10,20, ~ 100)  50 todo:
static int MotorDelay = 1 ; // test RTurn/LTurn time period
volatile boolean JSControl = false ; // default control by smart mobile devices   默认适用手机控制
//-----
volatile boolean PIDEnableFlag = true ; // default disable PID control 
...
void setup() 
{
  ...
  // Read Motor seeting from the EEPROM,If addr(0) = "AA" then read the presetting value,otherwise just skip   从EEPROM中读取缓存的小车速度
  ...
  tick.every(200, BatteryDetectHandler);// check battery status once 200ms  检查电池状态
  tick.every(1000, OneSecTimerHandler);// read temperature/humidity once per second  读取温湿度值
  if(DistanceDetect() > 0)  // 03/2015 add to check if ultrasound is exist or not ? if yes then start distance monitor  检查距离检测模组是否开启
  ...
  AppsduinoBot.registerFunction(JoyStickControl, 'j'); // setup the Joystick Control function     游戏杆控制
//---------------------------------------
...
void DCMotor(int LMspeed,int RMspeed,int dir)
{
   ...
   switch(dir) {
     ...
     MotorMode = TurnOff ;
     Serial.println("the dirction turn off:");
     break ;
    ...
    // both motors forward 
     ...
     MotorMode = FORWARD ;
     Serial.print("the dirction FORWARD:"); Serial.println(RMspeed);
...
void BTCommandHandler(byte flag, byte numOfValues)
{
      ...
      int randomvalue = AppsduinoBot.getInt();               //读取小车方向
...
void StartPDControl()
{
    Serial.println("start pdcontrol");
...
void PD_Control()
{
  ...
  Serial.print("  start pdcontrol  speedPinA-----------");Serial.println(RightMotorSpeed-power_difference);
  Serial.print("  start pdcontrol  speedPinB-----------");Serial.println(RightMotorSpeed+power_difference);
  Serial.print("  start pdcontrol  AspeedPin-----------");Serial.println(speedPinA);
   Serial.print("  start pdcontrol BspeedPin-----------");Serial.println(speedPinB);
...
{ 
  ...
  if(EventFlag & UltraSoundEvent ){  //---------- check the distance with UltraSound sensor ---- 
    ...
    USRange = DistanceDetect() ; // Get the ultrasound distance 超声距离
    if(USRange != 0 && USRange <= UltraSoundThreshold ){ // Check the distance of obstacle < 5cm then start some actions 
     ...
     DCMotor(HalfPowerMotor,HalfPowerMotor,BACKWARD); //Stop Motor  停止马达
     } 
 }  
}
